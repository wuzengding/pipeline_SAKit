...
rule run_pbflip_progress:
    input:
        collapsed_group = os.path.join(outpath,"results/03.Collapse/cupcake.collapsed.group.txt"),
        collapsed_obj_fa = rules.Isoform_calling.output[1],
        collapsed_filtered_obj_fa = os.path.join(outpath,"results/03.Collapse/cupcake.collapsed.min_fl_2.rep.fa"),
        collapsed_5prim_filtered_obj_fa = rules.Isoform_filter_subset.output[1],
        classification_filtered_obj_fa = rules.Isoform_classification_filter.output[5]
    output:
        os.path.join(outpath,"results/FinalResults/{0}_pb_isoform_filter_status.tsv".format(SampleID))

    threads:
        1

    log:
        os.path.join(outpath,"log/run_pbflip_progress.log")
    run:
