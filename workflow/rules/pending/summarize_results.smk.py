...
rule run_summarize_results:
    input:    
        final_results_file = rules.Isoform_summary.output[0],
        final_results_filtered_out_file = rules.Isoform_summary.output[1],
        pbsv_vcf_annot_tsv  = rules.SV_calling.output.pbsv_vcf_annot_tsv

    output:
        summary_result_files_sqanti3 = os.path.join(outpath,"results/FinalResults/{0}_sqanti3_classification_summary.tsv".format(SampleID)),
        summary_result_files_pbsv = os.path.join(outpath,"results/FinalResults/{0}_pbsv_annotation_summary.tsv".format(SampleID))


    
    params:
        sample_name = config["SampleID"]
    ...
    log:
        os.path.join(outpath,"log/run_summarize_results.log")
