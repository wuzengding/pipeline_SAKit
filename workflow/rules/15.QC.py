rule QC:
    ...
    log:
        os.path.join(outpath,"log/{0}.QC.log".format(SampleID))
    params:
    ...
    shell:
        ...
        -r {params.bedfile} -o {params.outpath}/{SampleID} >> {log} 2>&1
        ...
        -r {params.bedfile} -o {params.outpath}/{SampleID}.genome > {log} 2>&1
    
        {params.pytool} {params.rseqc}/FPKM_count.py -i {input.bamfile} \
        -r {params.bedfile} -o {params.outpath}/{SampleID}.wgs >> {log} 2>&1
         
        {params.pytool} {params.rseqc}/junction_saturation.py -i {input.bamfile} \
        -r {params.bedfile} -o {params.outpath}/{SampleID}.rRNA >> {log} 2>&1
        ...
        --RIBOSOMAL_INTERVALS {params.rRNA_interval} --STRAND_SPECIFICITY NONE >> {log} 2>&1
        """